(ns c3kit.wire.mock.memory-storage-spec
  (:require [c3kit.apron.corec :as ccc]
            [c3kit.wire.mock.memory-storage :as sut]
    ...
    (it "exceeds quota size"
      ...
      (try
        ...
        (catch js/Error e
          ...
          (should= "MemStorage quota has been exceeded. Quota: 5242880, Size: 5242882" (ccc/oget e "message")))))
  ...
  (context "Spec Helper"
    ...
    (context "with memory local storage"
      ...
      (it "redefines local storage methods"
        (should-not= (ccc/oget js/localStorage "getItem") (:get-item js-local-storage))
        (should-not= (ccc/oget js/localStorage "setItem") (:set-item js-local-storage))
        (should-not= (ccc/oget js/localStorage "removeItem") (:remove-item js-local-storage))
        (should-not= (ccc/oget js/localStorage "clear") (:clear js-local-storage))
    ...
    (context "with memory session storage"
      ...
      (it "redefines session storage methods"
        (should-not= (ccc/oget js/sessionStorage "getItem") (:get-item js-session-storage))
        (should-not= (ccc/oget js/sessionStorage "setItem") (:set-item js-session-storage))
        (should-not= (ccc/oget js/sessionStorage "removeItem") (:remove-item js-session-storage))
        (should-not= (ccc/oget js/sessionStorage "clear") (:clear js-session-storage))

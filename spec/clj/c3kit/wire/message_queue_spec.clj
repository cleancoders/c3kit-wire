...
(defn wait-until
  "Polls pred every 10ms until it returns truthy or the deadline passes.
   Replaces arbitrary sleeps in async tests so timing races can't fail the
   assertion that follows. Returns pred's value (nil on timeout)."
  ([pred] (wait-until pred 2000))
  ([pred timeout-ms]
        ...
        (let [errors        (atom 0)
          ...
          (letfn [(mock-xtrim [commands queue args]
            ...
            (with-redefs [redis/xtrim mock-xtrim]
              ...
              (wait-until #(.contains (log/captured-logs-str) (str ex)))
              (should= (str ex) (log/captured-logs-str))
              (wait-until #(empty? (sut/messages)))
              (should-be empty? (sut/messages))))))

### 4.3.0
 * Adds a secure-by-default service worker library for offline/PWA support, in two namespaces. `c3kit.wire.service-worker.core` provides the worker-side surface: a `cacheable?` predicate that is secure by default (caches only same-origin, ok, non-opaque GET responses and rejects `Cache-Control: no-store/no-cache/private`, `Vary: */Cookie/Authorization`, and credentialed requests), five caching strategies (`cache-first`, `network-first`, `stale-while-revalidate`, `network-only`, `cache-only`), a route registry (`register-route!`, `set-default!`, `precache!`), lifecycle handlers (`install`, `activate`, `handle-fetch`), and `start!` to wire them onto the global scope. `c3kit.wire.service-worker.register` provides the page-side entry points (`register!` / `unregister!`, and the injectable `register-with` / `unregister-with`) with `:on-update` / `:on-active` callbacks. Documented limitations: the `Set-Cookie` guard cannot fire in a live browser (forbidden response-header name), and same-origin cookie-session responses are not auto-detected — route those to `network-only`; `:allow-cross-origin` and `:cache-credentialed` are explicit, deliberately-dangerous opt-ins. In-memory test doubles are shipped in `c3kit.wire.service-worker.fake` so consumers can drive the worker synchronously in specs.
